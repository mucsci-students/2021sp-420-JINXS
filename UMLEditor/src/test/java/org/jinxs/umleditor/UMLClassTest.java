...
public class UMLClassTest  {
    ...
    // --------------------------------------------
    // getAttrs is effectively tested while testing add, delete, and rename
    ...
    public void addAttrTest() {
        ...
        assertTrue("Attributes ArrayList is empty", c.getAttrs().isEmpty());

        // Add a single attr
        c.addAttr("attr1");
        assertEquals("Attributes ArrayList has size 1", c.getAttrs().size(), 1);
        assertEquals("Attribute is added correctly", c.getAttrs().get(0), "attr1");

        // Add a second attr
        c.addAttr("attr2");
        assertEquals("Attributes ArrayList has size 2", c.getAttrs().size(), 2);
        assertEquals("Attribute is added correctly", c.getAttrs().get(1), "attr2");

        // Add two more attrs
        c.addAttr("attr3");
        c.addAttr("attr4");
        assertEquals("Attributes ArrayList has size 4", c.getAttrs().size(), 4);
        assertEquals("Attribute is added correctly", c.getAttrs().get(2), "attr3");
        assertEquals("Attribute is added correctly", c.getAttrs().get(3), "attr4");

        // Make sure that the capacity is ensured when exceeding 100 attrs
        for (int i = 5; i < 105; ++i) {
            c.addAttr("attr" + i);
        }
        assertEquals("Attributes ArrayList has size 104", c.getAttrs().size(), 104);
        assertEquals("Attribute is added correctly", c.getAttrs().get(103), "attr104");

        // Make sure duplicates are not allowed
        assertTrue("addAttr returns false when adding dup", !c.addAttr("attr1"));
        assertEquals("Attributes ArrayList has size 104", c.getAttrs().size(), 104);
        assertEquals("Attributes stays the same when adding dup", c.getAttrs().get(0), "attr1");
        assertEquals("Attributes stays the same when adding dup", c.getAttrs().get(103), "attr104");
        ...
        assertEquals("Attributes stays the same when adding dup", c.getAttrs().get(0), "attr1");
        assertEquals("Attributes stays the same when adding dup", c.getAttrs().get(103), "attr104");
        assertEquals("Attributes stays the same when adding dup", c.getAttrs().get(52), "attr53");
    }
    ...
    public void deleteAttrTest() {
        ...
        assertEquals("Attributes ArrayList has size 1", c.getAttrs().size(), 1);
        c.deleteAttr("attr1");
        assertEquals("Attributes ArrayList has size 0", c.getAttrs().size(), 0);
        ...
        c.deleteAttr("attr2");
        assertEquals("Attributes ArrayList has size 1", c.getAttrs().size(), 1);
        assertEquals("Attribute stays in the list when another is deleted", c.getAttrs().get(0), "attr1");

        // Add 2 more attrs and remove the 1st from the previous test
        c.addAttr("attr3");
        c.addAttr("attr4");
        assertEquals("Attributes ArrayList has size 3", c.getAttrs().size(), 3);
        c.deleteAttr("attr1");
        assertEquals("Attributes ArrayList has size 2", c.getAttrs().size(), 2);
        assertEquals("Attribute stays in the list when another is deleted", c.getAttrs().get(0), "attr3");
        assertEquals("Attribute stays in the list when another is deleted", c.getAttrs().get(1), "attr4");

        // Add 100 more attrs and delete all but one
        for (int i = 5; i < 105; ++i) {
            c.addAttr("attr" + i);
        }
        assertEquals("Attributes ArrayList has size 102", c.getAttrs().size(), 102);
        assertEquals("First attr is correct after adding many attrs", c.getAttrs().get(0), "attr3");
        assertEquals("Last attr is correct after adding many attrs", c.getAttrs().get(101), "attr104");

        for (int i = 3; i < 104; ++i) {
            c.deleteAttr("attr" + i);
        }
        assertEquals("Attributes ArrayList has size 1", c.getAttrs().size(), 1);
        assertEquals("First attr is correct after deleting many attrs", c.getAttrs().get(0), "attr104");
    }
    ...
    public void renameAttrTest() {
        ...
        assertEquals("Attributes ArrayList has size 1", c.getAttrs().size(), 1);
        c.renameAttr("attr1", "attr2");
        assertEquals("Attributes ArrayList has size 1 after renaming", c.getAttrs().size(), 1);
        assertEquals("First attr is correct after renaming", c.getAttrs().get(0), "attr2");
        ...
        assertEquals("Attributes ArrayList has size 3 after rename", c.getAttrs().size(), 3);
        assertEquals("First attr is correct after renaming third", c.getAttrs().get(0), "attr2");
        assertEquals("Second attr is correct after renaming others", c.getAttrs().get(1), "attr3");
        assertEquals("Third attr is correct after renaming", c.getAttrs().get(2), "attr5");
        ...
        c.deleteAttr("attr2");
        c.deleteAttr("attr3");
        assertEquals("Attributes ArrayList has size 0 after deleting", c.getAttrs().size(), 0);
        ...
        assertEquals("Attributes ArrayList has size 107", c.getAttrs().size(), 107);
        ...
        assertEquals("First attr is correct after renaming when attrs > 100", c.getAttrs().get(0), "coolAttr");
        assertEquals("Last attr is correct after renaming when attrs > 100", c.getAttrs().get(106), "coolerAttr");
    }
}